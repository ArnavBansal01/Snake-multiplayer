  ...
  function tickRoom(roomCode: string, io: Server): void {
    ...
    for (const pid of playerIds) {
      ...
      // --- Record history (WITH INTERPOLATION) ---
      let lastRecord = snake.history[0];
      let distSinceLast = Math.hypot(snake.head.x - lastRecord.x, snake.head.y - lastRecord.y);

      // If we moved a large distance in one tick, fill in the missing history points
      while (distSinceLast >= RECORD_DIST) {
        const angle = Math.atan2(snake.head.y - lastRecord.y, snake.head.x - lastRecord.x);
        
        // Calculate the exact point 4 pixels away from the last record
        const newX = lastRecord.x + Math.cos(angle) * RECORD_DIST;
        const newY = lastRecord.y + Math.sin(angle) * RECORD_DIST;
        
        const newPoint = { x: newX, y: newY };
        snake.history.unshift(newPoint);
        
        // Update for the next iteration of the loop
        lastRecord = newPoint;
        distSinceLast = Math.hypot(snake.head.x - lastRecord.x, snake.head.y - lastRecord.y);
      }
    ...
    for (let i = room.foods.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(snake.head.x - f.x, snake.head.y - f.y) < 20 + f.size) {
        ...
        snake.score += f.value;
        // Divides growth by 4x
        snake.length = 10 + Math.floor(snake.score / 40);
        // Divides speed acceleration by 4x
        snake.speed += (SPEED_INC * 0.25);
        // -----------------------------
    ...
    // --- Broadcast state (OPTIMIZED PAYLOAD) ---
  const statePayload: Record<string, any> = {};
  for (const pid of Object.keys(room.players)) {
    const pr = room.players[pid];
    
    // Compress history by rounding floats to 1 decimal place.
    // This reduces the socket payload size by over 60%!
    const optimizedHistory = pr.snake.history.map(p => ({
    ...
    statePayload[pid] = {
      snake: {
        ...pr.snake,
        head: { 
          x: Math.round(pr.snake.head.x * 10) / 10, 
          y: Math.round(pr.snake.head.y * 10) / 10 
        },
  ...
  // Compress foods array
  const optimizedFoods = room.foods.map(f => ({
    ...f,
    x: Math.round(f.x),
    y: Math.round(f.y)
  }));
 ...
 function handleDeath(roomCode: string, playerId: string, killedBy: string, io: Server): void {
    ...
    // --- NEW: Drop continuous super food along the entire body trail ---
    const segmentPositions: Point[] = [];
    for (let i = 0; i < snake.length; i++) {
      const idx = i * SEGMENT_SPACING_IDX;
      if (idx < snake.history.length) {
        segmentPositions.push(snake.history[idx]);
      }
    }

    // Interpolate between segments to make a solid line of food
    for (let i = 0; i < segmentPositions.length - 1; i++) {
      const p1 = segmentPositions[i];
      const p2 = segmentPositions[i + 1];
      const dist = Math.hypot(p2.x - p1.x, p2.y - p1.y);
      
      // Drop a food every 12 pixels (creates a seamless overlapping line)
      const drops = Math.max(1, Math.floor(dist / 12));
      
      for (let j = 0; j < drops; j++) {
        const t = j / drops;
        const x = p1.x + (p2.x - p1.x) * t;
        const y = p1.y + (p2.y - p1.y) * t;
        spawnFood(1, room.foods, x, y, true); 
      }
    }
    // -------------------------------------------------------------------
      ...
      if (pr) {
        ...
        for (let i = 0; i < pr.snake.length; i++) {
          ...
          if (idx < pr.snake.history.length) {
            segmentPositions.push(pr.snake.history[idx]);
          }
        }
